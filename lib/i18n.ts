...
const s: Record<string, Record<Lang, string>> = {
  ...
  proj_ecu_desc:      { en: "Built an ECU software testing app to compare expected vs. actual behavior in normal and failure scenarios, with fault simulation and pass/fail safety results using car simulation.", fr: "Développement d'une application de test logiciel ECU pour comparer le comportement attendu au comportement réel en scénarios normaux et de défaillance, avec simulation de défauts et résultats de sécurité pass/fail via simulation de véhicule.", ko: "정상 및 고장 시나리오에서 예상 동작과 실제 동작을 비교하는 ECU 소프트웨어 테스트 앱을 개발했으며, 차량 시뮬레이션 기반의 결함 시뮬레이션과 pass/fail 안전성 결과를 제공했습니다." },
